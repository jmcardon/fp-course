...
sequence ::
  ...
  List (f a)
  -> f (List a)
sequence = foldRight (\l r -> pure (:.) <*> l <*> r) (pure Nil)
...
replicateA n fa
  | n > 0 = pure (:.) <*> fa <*> replicateA (n-1) fa
  | otherwise = replicateA 1 fa
...
filtering ::
  ...
  -> List a
  -> f (List a)
filtering f = foldRight (\l r -> pure (\x -> if x then (l:.) else id) <*> (f l) <*> r) (pure Nil) -- kind of sucks, evals both args
