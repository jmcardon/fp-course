...
-- Note: Can also be map + flatten
flatMap ::
  (a -> List b)
  -> List a
  -> List b
flatMap _ Nil       = Nil
flatMap f (x:.rest) = (f x)++(flatMap f rest)
...
seqOptional ((Full a):.rest) = composeList a (seqOptional rest)
  where composeList _ Empty    = Empty
        composeList x (Full y) = Full(x:.y)
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 l = count 0 l
  where count n Nil = n > 4
        count 4 _ = True
        count n (_:.xs) = count (n+1) xs
-- lengthGT4 l = length > 4
