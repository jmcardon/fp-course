...
hex ::
  Parser Char
hex = thisMany 4 (satisfy isHexDigit) >>= (\a -> case readHex a of
  Full a -> pure (chr a)
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 p1 p2 = list inter >>= (\a -> case a of
  Nil -> pure <$> p1
  _   -> (a ++). pure <$> p1 ||| pure a)
  where
    inter = do
...
eof ::
  Parser ()
eof = P(\c -> if isEmpty c then Result Nil () else UnexpectedString c)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll li = satisfy $ and . (<$> li) . flip ($)
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny li = satisfy $ or . (<$> li) . flip ($)
