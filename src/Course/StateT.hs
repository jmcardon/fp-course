...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f a
    -> StateT s f b
  (<$>) f st = StateT (\s -> (\(x, y) -> (f x, y)) <$> runStateT st s)
...
distinct' ::
  ...
  List a
  -> List a
distinct' li = eval' (filtering (\a -> (StateT(\s -> ExactlyOne (not $ S.member a s, S.insert a s)))) li) S.empty
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF li = evalT (filtering (\a -> StateT(\s -> if(a > 100) then Empty else Full (not $ S.member a s, S.insert a s))) li) S.empty
...
instance Functor f => Functor (OptionalT f) where
  (<$>) f fa = OptionalT ((\o -> f <$> o) <$> runOptionalT fa)
...
instance Applicative (Logger l) where
  pure = (Logger Nil)
  (<*>) (Logger li f) (Logger lo fa) = Logger (li ++ lo) (f fa)
...
instance Monad (Logger l) where
  (=<<) f (Logger li a) = case (f a) of (Logger lo b) -> Logger (li ++ lo) b
...
distinctG li =
  runOptionalT (evalT (filtering (\a -> StateT(\s ->
    OptionalT (if a > 100 then log1 (fromString("aborting > 100: " P.++ show a)) Empty
      else if (even a) then log1 (fromString ("even number: " P.++ show a)) (Full (not $ S.member a s, S.insert a s))
        else pure (Full(not $ S.member a s, S.insert a s))
    ))) li) S.empty)
